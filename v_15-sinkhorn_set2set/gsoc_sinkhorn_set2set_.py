...
Model 15
==============================================================
BEFORE RUNNING: edit the four paths in the CONFIG block below to match
your machine. Everything else can be left as-is.
...
# ══════════════════════════════════════════════════════════════════════════
# >>> EDIT THESE 2 PATHS FOR YOUR MACHINE <<<
# ══════════════════════════════════════════════════════════════════════════
JETNET_DATA_DIR = "/pscratch/sd/r/rushil13/jetnet_data"

# Where all outputs (checkpoints, weights, plots, logs) get written.
SAVE_DIR = "/pscratch/sd/r/rushil13/gsoc_rushil/results_v15"

# Sub-folders under SAVE_DIR — usually fine to leave these as-is.
...
with open(LOG_FILE, "w") as f:
    f.write("=== Pipeline V5 Model15: SpatialSet2Set + NO bottleneck + (SlotDecoder+self-attention) + Sinkhorn ===\n\n")
...
GNN_HIDDEN_DIM      = 256
GNN_K               = 10     # Increased from 6 → 10: safe with Stable ChebNet
CHEB_STEP_SIZE      = 0.45   # Euler step size ε (from EulerConv config)
CHEB_DISSIPATION    = 0.1    # Dissipative force g (shifts eigenvalues left)
KNN_K               = 10
TOTAL_PARTICLES     = 150
SET2SET_STEPS       = 4
POS_DIM             = 64
...
# FLOW_PATH = "/pscratch/sd/r/rushil13/gsoc_rushil/results_v15/checkpoints/flow_epoch_latest.pth"
...
#
# Why: Standard PyG ChebConv has unconstrained weight matrices per polynomial
# order k. At large K the Chebyshev recurrence amplifies errors and Jacobian
# eigenvalues escape the unit circle → exploding/vanishing features.
#
# Fix (from UseCase_StableCheb/EulerConv.py):
#   1. AntiSymmetric: forces W = W_upper - W_upper^T - g*I
#      → purely imaginary eigenvalues (energy-preserving) + g*I dissipation
#   2. Euler step: out = x + ε * ChebConv(x)
#      → treats the layer as ODE step dx/dt = f(x), keeps |Jacobian| ≈ 1
# This lets us safely use K=10–20+ for wider neighbourhood aggregation.
# ══════════════════════════════════════════════════════════════════════════════

class AntiSymmetric(Module):
    """Parametrize a weight matrix as W = W_upper - W_upper^T - g*I.
    Eigenvalues are constrained to the left half-plane:
      - Anti-symmetric part → purely imaginary eigenvalues (no energy loss/gain)
      - Dissipative -g*I    → real parts ≤ -g (controlled decay)
    """
...
class Euler_ChebConv(MessagePassing):
    """Stable ChebConv via anti-symmetric weight parametrization + Euler step.

    Computes:  out = x  +  ε · Σ_{k=0}^{K-1} W_k · T_k(L̃) · x

    where every W_k is constrained to be anti-symmetric (minus dissipation),
    guaranteeing that the Jacobian eigenvalues stay near the unit circle.
    This makes high K values (10, 15, 20) numerically stable.

    Args:
        in_channels:       Input feature dimension.
        out_channels:      Output feature dimension.
        K:                 Chebyshev polynomial order (= receptive field hops).
        step_size:         Euler step ε. Smaller → more stable, less expressive.
        dissipation_force: g ≥ 0. Larger → faster decay, more stable but more lossy.
        bias:              Add learnable bias after the sum.
    """
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
        ...
        # K separate weight matrices, each constrained anti-symmetric
    ...
    def _norm(self, edge_index, num_nodes, edge_weight, dtype, batch=None):
        """Compute 2*L/λ_max - I (same as PyG ChebConv normalization)."""
    ...
    def forward(self,
        ...
        # Chebyshev recurrence: T_0=x, T_1=L̃x, T_k=2L̃T_{k-1}-T_{k-2}
        Tx_0 = x
        Tx_1 = x  # placeholder
        ...
        # ── Euler step: the key stability guarantee ────────────────────────
        # Instead of replacing x (like vanilla ChebConv), we take a small
        # step ε in the direction of the aggregated signal.  This is
        # equivalent to one forward-Euler step of  dx/dt = f(x, G).
        return x + self.e * out
...
# ══════════════════════════════════════════════════════════════════════════════
# DATA LOADING  — no monkeypatch, JetNet handles download itself
# ══════════════════════════════════════════════════════════════════════════════
...
def load_jetnet_data():
    ...
    for tid, jtype in enumerate(['g', 'q', 't']):
        p, j = JetNet.getData(
            ...
            num_particles=TOTAL_PARTICLES, split='train',
            download=True,
        )
...
# ══════════════════════════════════════════════════════════════════════════════
# MODEL 1: SPATIAL SET2SET ENCODER
# ══════════════════════════════════════════════════════════════════════════════

class SpatialSet2SetChebNet(nn.Module):
    """GNN encoder using Stable ChebNet (Euler_ChebConv) + Set2Set pooling.

    Replaces vanilla PyG ChebConv with Euler_ChebConv:
      - Anti-symmetric weight matrices → bounded eigenvalues
      - Euler residual step → Jacobian modulus ≈ 1.0
    This allows K=10 (was 6) without gradient explosion, giving each particle
    a 10-hop receptive field which is near-global on JetNet-150 graphs.

    Architecture:
      conv1: input (3D) → hidden (256D)  — initial feature projection
      conv2: hidden → hidden              — 2-hop aggregation (stable)
      conv3: hidden → hidden              — 3-hop aggregation
      conv4: hidden → hidden (L2-norm)    — final feature normalization layer
      pos_mlp: (η,φ) coordinates → pos_dim spatial embedding
      fuse_proj: concat(node_feat, pos_emb) → hidden
      pool: Set2Set (4 LSTM steps) → 2×hidden = 512-dim latent
    """
    def __init__(self, in_dim=3, hidden=GNN_HIDDEN_DIM, K=GNN_K,
        ...
        super().__init__()
        # ── Input projection (in_dim → hidden) done as a plain nn.Linear BEFORE
        # Euler_ChebConv so that all 4 conv layers are SQUARE (hidden×hidden).
        # AntiSymmetric parametrization requires square weight matrices:
        # W = W_upper - W_upper^T - g*I  only makes sense when in == out.
        # Without this, conv1(3→256) would throw a shape-mismatch at runtime.
        self.input_proj = nn.Linear(in_dim, hidden, bias=False)

        # All 4 Euler_ChebConv layers are now square (hidden → hidden)
        ...
        # BatchNorm after each conv (applied before activation)
    ...
    def forward(self, x, edge_index, batch=None):
        coords = x[:, :2]   # (η, φ) particle coordinates

        # Project raw 3-dim particle features (η, φ, pT) → hidden-dim space.
        # This is done BEFORE the graph convolutions so that all Euler_ChebConv
        # layers operate on square hidden×hidden weight matrices.
        h = self.input_proj(x)                                  # [N, hidden]

        # Euler_ChebConv already contains an internal residual (out = x + ε·f(x)),
        # so BN+activation is applied to its output, not the raw convolution.
...
# ══════════════════════════════════════════════════════════════════════════════
# MODEL 2: SLOT DECODER WITH IDENTITY EMBEDDINGS (concatenated, not added)
# ══════════════════════════════════════════════════════════════════════════════
...
class SlotDecoder(nn.Module):
    def __init__(self, latent_dim=LATENT_DIM,
        ...
        # ── Self-attention: lets the 150 slots "see" each other before the
        # shared MLP runs, so they can spread out and avoid duplicate
        # particle predictions (see conversation for why this matters).
        # Operates on the concatenated [z_broadcast, slot_id] vectors,
        # so d_model = slot_dim * 2.
        d_model = slot_dim * 2
        ...
        self.shared_net = nn.Sequential(
            nn.Linear(slot_dim * 2, hidden),   # *2: concat(z_proj, slot_id), not sum
            nn.LayerNorm(hidden),
            nn.GELU(),
            nn.Dropout(0.1),
    ...
    def forward(self, z):
        ...
        ids = self.slot_id_emb(self.slot_ids)            # [150, slot_dim]
        ids = ids.unsqueeze(0).expand(B, -1, -1)         # [B,   150, slot_dim]
        slots = torch.cat([z_broadcast, ids], dim=-1)  # [B, 150, slot_dim*2]

        slots = self.self_attn(slots)                   # [B, 150, slot_dim*2] — slots refined via mutual attention

        flat  = slots.view(B * self.n_particles, self.slot_dim * 2)
        out   = self.shared_net(flat)
        out   = out.view(B, self.n_particles, self.particle_dim)
...
def sinkhorn_3d_loss(pred, target, pt_scale=PT_SCALE):
    pred_pts = torch.cat([
        pred[:, :, 0:1], pred[:, :, 1:2], pred[:, :, 2:3] * pt_scale
    ], dim=-1).contiguous()
    tgt_pts = torch.cat([
        target[:, :, 0:1], target[:, :, 1:2], target[:, :, 2:3] * pt_scale
    ], dim=-1).contiguous()
...
class JetGraphDataset(Dataset):
    def __init__(self, graphs, targets):
        self.graphs = graphs; self.targets = targets
    def __len__(self):
...
def train_autoencoder(gnn, decoder, graph_data, target_particles,
    ...
    if os.path.exists(ENC_PATH) and os.path.exists(DEC_PATH):
        ...
        gnn.load_state_dict(torch.load(ENC_PATH, map_location=device), strict=False)
    ...
    log_print(f"\n=== AE Training (Model 15) — NO BOTTLENECK ===")
    log_print(f"    Encoder  : SpatialSet2SetChebNet → [B, {LATENT_DIM}]")
    log_print(f"    Decoder  : SlotDecoder (slot_dim={SLOT_DIM}, concat identity) → [B, 150, 4]")
    log_print(f"    Loss     : Sinkhorn(p=1, blur=0.05, pT×{PT_SCALE}) + mult_bce")
    log_print(f"    Epochs   : {start_epoch}→{epochs}  |  {n_batches} batches/epoch")
    ...
    plt.title('AE Loss — SpatialSet2Set + SlotDecoder + Sinkhorn(p=1)')
    plt.grid(True)
...
# ══════════════════════════════════════════════════════════════════════════════
# MODEL 3: CONDITIONAL FLOW MATCHING
# ══════════════════════════════════════════════════════════════════════════════
...
def train_flow_matching(embeddings, jet_features, jet_type_ids,
    ...
    if os.path.exists(FLOW_PATH) and start_epoch == 0:
        log_print("Found trained flow weights — loading, skipping training.")
        model.load_state_dict(torch.load(FLOW_PATH, map_location=device), strict=False)
        model.eval()
        return model
    ...
    plt.title('Flow Matching Loss — 512-dim latent')
    plt.grid(True)
...
def plot_distributions(real_particles, gen_particles, save_name):
    ...
    fig.suptitle('Real vs Generated — Model15 (SpatialSet2Set + SlotDecoder + Sinkhorn p=1)',
                 fontsize=12)
    plt.tight_layout()
...
def main_pipeline():
    log_print("\n" + "=" * 70)
    log_print("PIPELINE V5 MODEL 15: SpatialSet2Set + SlotDecoder + Flow(512) + Sinkhorn(p=1)")
    log_print("=" * 70 + "\n")

    particle_data, jet_data, jet_types_all = load_jetnet_data()

    # Using the complete dataset — no capping/subsampling. Every g, q, and
    # t jet that JetNet returns gets processed.
    graph_data, target_particles, original_jets, collected_types = \
    ...
    flow_model = train_flow_matching(z_norm, jet_features, jet_type_ids, epochs=EPOCHS_DIFF)
    ...
    visualize_jets(gen_vis, titles, save_name="generated_jets_v5_m15")
    ...
    with torch.no_grad():
        for i in range(0, len(z_gen_denorm), BATCH_SIZE_FLOW):
            zb = torch.tensor(z_gen_denorm[i:i+BATCH_SIZE_FLOW], dtype=torch.float32).to(device)
    ...
    plot_distributions(real_parts, gen_parts, "dist_comparison_model15")
